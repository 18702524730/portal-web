...
<script>
  ...
  import DemandBrandCard from '../card/DemandBrandCard'

  export default {
    name: 'DemandBrandTable',
    components: { DemandBrandCard, OrderButton },
    props: {
      demandOrSupply: {
        type: String,
        default: 'demand'
    ...
    created() {
      if (this.demandOrSupply === 'supply') {
        this.formParam.supplyRequirementType = '3'
      }
    ...
    mounted() {
      const that = this
      bus.$on('changeParamSupplyBrand', function(param) {
        that.ipagination.current = 1
        const newParam = that.handleParam(param)
        that.loadData(newParam)
      })
      bus.$on('changeParamDemandBrand', function(param) {
        that.ipagination.current = 1
        const newParam = that.handleParam(param)
        that.loadData(newParam)
      })
    ...
    beforeDestroy() {
      ...
      bus.$off('changeParamSupplyBrand')
      bus.$off('changeParamDemandBrand')
    },
    methods: {
      refresh() {
        const param = this.handleParam(this.formParam)
