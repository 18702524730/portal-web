    ...
    <div class="container">
      <a-spin tip="加载中..." :spinning="spinning">
        <a-form :form="filter">
          ...
          <a-form-item label="商标分类:" :label-col="labelCol" :wrapper-col="wrapperCol">
            <a-select placeholder="请选择商标类别" style="width: 200px" v-model="filter.trademarkType">
              <template v-for="(item,index) in dictList.trademarkType.dictOptions">
          ...
          <a-form-item label="所属行业:" :label-col="labelCol" :wrapper-col="wrapperCol" v-if="demandOrSupply === 'supply'">
          ...
          <a-form-item label="组合类型:" :label-col="labelCol" :wrapper-col="wrapperCol">
            <a-radio-group buttonStyle="solid" v-model="filter.combinationCategory">
              <template v-for="(item,index) in dictList.combinationCategory.dictOptions">
                <a-radio-button :value="item.value" :key="index">{{item.text}}</a-radio-button>
              </template>
            </a-radio-group>
  ...
  export default {
    ...
    created() {
      if (this.demandOrSupply === 'supply') {
        ...
        this.filter.supplyRequirementType = '3'
      }
      this.spinning = false
    ...
    methods: {
      ...
      changeParam(param) {
        if (this.demandOrSupply === 'demand') {
          bus.$emit('changeParamDemandBrand', param)
        } else if (this.demandOrSupply === 'supply') {
          bus.$emit('changeParamSupplyBrand', param)
        }
      },
      initDictData(item) {
        // 根据字典Code, 初始化字典数组
      ...
      handleClose(tag) {
        ...
        switch (tag.key) {
          case 'trademark_type':
            this.filter.trademarkType = undefined
            break
          case 'combination_category':
            this.filter.combinationCategory = ''
            break
          case 'pro_amout':
    ...
    watch: {
      'filter.belongIndustry': {
        handler(newData, oldData) {
          console.info(this.$refs.myDict.treeValue)
      ...
      'filter.trademarkType': {
        handler(newData, oldData) {
          const a = this.dictList.trademarkType.dictOptions.filter(item => item.value === newData)[0]
          if (a !== undefined) {
            const arg = this.tags
            if (oldData !== undefined && oldData !== '') {
              for (let i = 0; i < arg.length; i++) {
                if (arg[i].key === 'trademark_type') {
                  arg.splice(i, 1)
                }
              }
            }
            a.key = 'trademark_type'
            arg.push(a)
            this.changeParam(this.filter)
          }
        }
      },
      'filter.combinationCategory': {
        handler(newData, oldData) {
          const a = this.dictList.combinationCategory.dictOptions.filter(item => item.value === newData)[0]
          if (a !== undefined) {
            const arg = this.tags
            if (oldData !== undefined && oldData !== '') {
              for (let i = 0; i < arg.length; i++) {
                if (arg[i].key === 'combination_category') {
                  arg.splice(i, 1)
                }
              }
            }
            a.key = 'combination_category'
            arg.push(a)
            this.changeParam(this.filter)
          }
        }
