  ...
  <div id="integrationAddModal">
    <a-modal
      ...
      <a-form-model
        ...
        <a-form-model-item label="权利人" prop="obligee">
          <a-input v-model="form.obligee" placeholder="请输入权利人"></a-input>
        </a-form-model-item>

        <a-form-model-item label="布图名称" prop="name">
          <a-input v-model="form.name" placeholder="请输入布图名称"></a-input>
        </a-form-model-item>

        <a-form-model-item label="布图文件" prop="imgUrl" :wrapper-col="wrapperInnerCol">
          <j-more-upload fileType="jpg,jpeg,png,bmp,pdf" fileSize="2" fileNum="5" v-model="form.imgUrl"
    ...
    data() {
      return {
        ...
        rules: {
          ...
          name: [{
            required: true, max: 40, message: '请输入布图名称,长度在40字以内!'
          }],
          imgUrl: [{
            required: true, message: '请上传布图文件!'
          }]
        },
      ...
      onSubmit() {
        this.$refs.ruleForm.validate(valid => {
          if (valid) {
            ...
            this.$emit('func', 'integration', this.form)
            this.handleCancel()
          } else {
