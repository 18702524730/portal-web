<template>
  <div id="supplyShowCard">
    <a-card hoverable :bordered="bordered" @click="handleClick(item)">
      <a-row class="head-my-row">
        <a-col>
          <div style="height: 100px;width: 100px; margin: 22px auto 19px;padding:5px 5px 10px"
               v-if="demandOrSupply === 'demand'">
            <a-avatar
              alt="无图片"
      ...
      <a-row class="head-row">
        <a-col>
          <div class="div-list">
            <p class="title-p">
              <span class="span-title">
                <a-tooltip placement="right" :title="item[`${itemKey[1]}`]"
                  ...
                  <a-tag color="#108ee9" class="my-tag">
                    {{ projectTypeName }}
                  </a-tag>
                  {{item[`${itemKey[1]}`] | ellipsis}}
                </a-tooltip>
              </span>
            </p>
            ...
            <p v-if="searchData"><span class="span-data" style="-webkit-box-orient: vertical;">{{column[1]}}:<a-tooltip
              placement="right"
              :title="item[`${itemKey[3]}`]">{{item[`${itemKey[3]}`] | formattingData | ellipsis2}}</a-tooltip></span>
            </p>
            <p v-if="searchData"><span class="span-data" style="-webkit-box-orient: vertical;">{{column[4]}}:<a-tooltip
              placement="right"
              :title="item[`${itemKey[8]}`]">{{item[`${itemKey[8]}`] | formattingData | ellipsis2}}</a-tooltip></span>
            </p>
            <p v-if="searchData"><span class="span-data" style="-webkit-box-orient: vertical;">{{column[2]}}:<a-tooltip
              placement="right"
              :title="tradingWay">{{tradingWay | formattingData | ellipsis2}}</a-tooltip></span>
            </p>
