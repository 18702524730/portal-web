        ...
        <a-row :gutter="16" type="flex" justify="space-between">
          <a-col :style="{width:demandOrSupply === 'supply'?'26.2%':'25%'}">
            ...
            <div v-else-if="demandOrSupply === 'supply'" style="height: 156px;width: 100%;">
              <img
                alt="无图片"
                :src="getImgNewView(item[`${itemKey[7]}`])"
                :onerror="defaultImg"
          ...
          <a-col :style="{width:demandOrSupply === 'supply'?'73.8%':'75%'}">
            <a-row>
              <a-col :span="12">
                <span class="span-title">{{item[`${itemKey[0]}`]}}</span>
              </a-col>
            ...
            <a-row class="my-row">
              <a-col>
                <a-row>
                  <a-col :span="12">
                    <p>{{itemTitle[0]}}：
                      <a-tooltip placement="top" :title="item[`${itemKey[1]}`]">{{item[`${itemKey[1]}`] |
                        formattingData}}
                      </a-tooltip>
                    </p>
                    <p v-if="itemTitle[1] !== '项目成熟度'">{{itemTitle[1]}}：
                      <a-tooltip placement="top" :title="item[`${itemKey[2]}`]">{{item[`${itemKey[2]}`] |
                        formattingData}}
                      </a-tooltip>
                    </p>
                    <p>{{itemTitle[2]}}：
                      <a-tooltip placement="top" :title="item[`${itemKey[3]}`]">{{item[`${itemKey[3]}`] }}</a-tooltip>
                    </p>
                  </a-col>
                  <a-col :span="12">
                    <p>{{itemTitle[3]}}：
                      <a-tooltip placement="top" :title="item[`${itemKey[4]}`]">{{item[`${itemKey[4]}`] |
                        formattingData}}
                      </a-tooltip>
                    </p>
                    <p>{{itemTitle[4]}}：<span style="color:#ff3300"
                                              v-text="itemKey[5] === 'money'?newMoney:item[`${itemKey[5]}`]"></span></p>
                    <p v-if="demandOrSupply === 'supply' && statusMes">状态：<span>{{statusMes}}</span></p>
                  </a-col>
                </a-row>
              </a-col>
            </a-row>
            <a-row>
              <p style="float: right" class="time-p">{{itemTitle[5]}}：{{item[`${itemKey[6]}`]}}</p>
            </a-row>
            <a-row>
    ...
    methods: {
      ...
      buyOrder(item) {
        if (this.statusMes === '已售出') {
          this.$message.warning('该专利已售出，正在对接中，无法购买')
          return false
        }
        if (this.statusMes === '对接中') {
          this.$message.warning('该专利正在对接中，无法购买')
          return false
        }
        if (this.statusMes === '待确认') {
          this.$message.warning('该专利处于待确认，无法购买')
          return false
        }
        if (judgeState()) {
          if (isAdmin()) {
